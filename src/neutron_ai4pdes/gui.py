...
class DifusaoGUI_AI4PDEs:
    ...
    def criar_controles(self):
        ...
        self.metodo_label = ttk.Label(
            self.painel_controles,
            text="Método: AI4PDEs 1D adaptado com stencils convolucionais fixos"
        )
...
"""
        ...
        try:
            ...
            if N < min_N:
                messagebox.showwarning(
                    "Aviso",
                    f"N deve ser maior ou igual ao número de regiões ({min_N})."
                )
                return
            ...
            self.solver = SolverDifusaoAI4PDEs(
                L=L, N=N, materiais=self.materiais_atual,
            ...
            def executar():
                try:
                    k_eff, phi = self.solver.resolver()
    ...
    def gerar_relatorio(self):
        ...
        if not REPORTLAB_AVAILABLE:
            messagebox.showwarning("Aviso", "ReportLab não está instalado.\nInstale com: pip install reportlab")
            return
        ...
        try:
            ...
            story.append(PageBreak())
            story.append(Paragraph("9. Conclusão Parcial", styles['Heading2']))
            ...
            conclusao += (
                " O cálculo foi realizado exclusivamente pelo solver AI4PDEs 1D adaptado implementado "
                "com stencils convolucionais fixos."
            )
